...
jQuery(function ($) {
	(function () {
		...
		const $toolbar = $('.toolbarFixed');
		const $mainNav = $('.mainNav');
		const $contWrap = $('#content_wrapper');
		const $contHead = $('#content_header');
		const mode = 'inline';
		...
		window.doResizeActions = function () {
			const maxY = $wrap.offset().top + toolbarHeight;
			const viewY = $window.scrollTop() + $window.height();
